// Package main runs a full XLS-96 confidential MPT lifecycle against devnet over WebSocket.
//
...
func mergeInbox(client *websocket.Client, holder *wallet.Wallet, issuanceID string) error {
...
//
// The spendable balance comes from builder.GetSpendingBalance, the same reader the RPC
// example uses: LedgerQuerier is satisfied by both clients, so neither example needs its
// own. The inbox has no such helper, because no transaction can spend it until a
// ConfidentialMPTMergeInbox moves it into the spending balance, so it is decrypted here
// alongside the public amount this example prints next to it.
...
func mptoken(client *websocket.Client, address types.Address) (ledgerentries.MPToken, error) {
...
func issuanceID(client *websocket.Client, issuer types.Address) (string, error) {
	objects, err := client.GetAccountObjects(&account.ObjectsRequest{
		Account:     issuer,
